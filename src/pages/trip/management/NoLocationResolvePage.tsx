...
const NoLocationResolvePage = () => {
    ...
    return (
        <div css={containerStyle}>
            ...
            <TaskContextCard tone='red' icon={<MapPin size={18} strokeWidth={2.2} color={MANAGE_TOKENS.destructive} />}>
                GPS 정보가 없는 사진들이에요. 지도에서 위치를 선택하거나 비슷한 시간대 사진의 위치를 가져올 수 있어요.
            </TaskContextCard>
            ...
            <main css={bodyStyle}>
                ...
                {targets.length === 0 ? (
                    <div css={emptyStyle}>처리할 위치 없는 사진이 없어요</div>
                ) : (
                    <div css={listStyle}>
                        {targets.map((p, i) => (
                            <PhotoListItem
