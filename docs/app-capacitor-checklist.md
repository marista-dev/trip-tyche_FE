...
APK 직배포 사슬이 끝단까지 연결됐다.

```
npm run apk:publish 1.0.0
  → 빌드·검증·서명 → 태그 v1.0.0 → GitHub Release
  → CI가 OCI 업로드 + 구버전 삭제
  → 서버 /v1/app/config가 그 주소를 안내
  → 웹·앱의 설치/업데이트 버튼이 실제 APK로 연결
```

검증한 것:
- `updateUrl` → HTTP 200, `application/vnd.android.package-archive`, 6.4MB
- 웹 번들에 새 안내 문구 반영 (`앱으로 더 정확하게`, `앱 설치하기`)
- 릴리스 APK 실기기 설치 → 프로덕션 로그인 → 실제 데이터 렌더링

### 남은 작업

- [ ] **⚠️ OCI 버킷 목록 공개** — 인증 없이 `GET /triptyche-storage/`로 전체 오브젝트 키가
      조회된다. 사용자 사진 키가 그대로 드러나 개별 URL을 몰라도 전량 수집이 가능하다.
      OCI 콘솔에서 가시성을 `ObjectRead`(목록 차단)로 바꿀 것. 이미지 표시·APK 다운로드는
      그대로 동작한다. **APK 배포와 무관하게 우선순위가 높다.**
